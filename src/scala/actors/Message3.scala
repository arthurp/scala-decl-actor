...
class Message3[A1, A2, A3](owner : DeclActor) extends Message(owner) {
    ...
    def unapply(v:Any) : Option[(A1, A2, A3)] = v match {
        case ActorMessage(src, (a1, a2, a3)) if src eq this => Some((a1.asInstanceOf[A1], a2.asInstanceOf[A2], a3.asInstanceOf[A3]))
        case _ => None
    }
}

class SyncMessage3[R, A1, A2, A3](implicit owner : DeclActor, protected[this] val rClass : Manifest[R]) extends Message3[A1, A2, A3](owner) with SyncMessage[R] {
